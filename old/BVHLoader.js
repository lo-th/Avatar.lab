...
THREE.BVHLoader.prototype = {
    ...
    applyToModel: function ( model, clip, tPose, seq, ratio ) {
       ...
       // var hipPos = new THREE.Vector3();
        ...
        for( k = 0; k < lngS; k++ ){
                ...
                if( nodeTracks[i].length === 2 ){
                    ...
                    for( j = 0; j < lng; j ++ ){
                        ...
                        /*if( name === 'hip' ){
                            ...
                            //hipPos.setFromMatrixPosition( localMtx );

                            hipPos.setFromMatrixPosition( globalMtx );
                            hipPos.x = 0;
                            hipPos.z = 0;

                        }

                        if( name === 'rFoot' || name === 'lFoot' ){

                            testPos.setFromMatrixPosition( globalMtx );
                            testPos.x = 0;
                            testPos.z = 0;
                            //testPos.setFromMatrixPosition( localMtx ); 

                            distance = testPos.distanceTo( hipPos );

                            //console.log(testPos.y)

                            if( testPos.y < minY ) minY = testPos.y
                            //if( resultPos.y < minY ) minY = resultPos.y
            ...
            //rootMtx.setPosition( new THREE.Vector3( py+minY, 0, 0 ) );  
            //maxY = -leg

            //Math.abs(leg) / maxY;

            //rootMtx.setPosition( new THREE.Vector3( -leg, 0, 0 ) ); 
