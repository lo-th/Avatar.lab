...
main = {
    ...
    onLoadModel: function ( p ) {
        ...
        var mat = new THREE.MeshStandardMaterial({skinning:true});

        var skl = new THREE.SkeletonHelper( head );

        var t = meshs.head_BFMG1o_JAWcc_001;

        head.material = mat;
       // 

        for(var o in meshs){
            meshs[o].material.skinning = true;
        }

        /*var i = head.children.length
        while(i--){
            if(head.children[i].name === 'head_BFB_Neck_Bone') t = head.children[i]
        }*/

        var bones = head.skeleton.bones;
        var i, name, bone, lng = bones.length, v;
        for( i=0; i<lng; i++){

            bone = bones[i];
            if(bone.name === 'head_BFB_Neck_Bone') t = bone
            //bone.matrixAutoUpdate = false;

        }


        view.getScene().add(head)
        view.getScene().add(skl)

        console.log(t)

        t.rotation.y+=10*Math.torad
